...
public class RSLocalPath extends RSPath {
    ...
    public RSTile getNext() {
        if (!ctx.game.getMapBase().equals(base)) {
            if (ctx.client.getCollisionMaps() != null) {
                int[][] flags = ctx.client.getCollisionMaps()[ctx.game.getPlane()].getFlags();
                if (flags != null) {
    ...
    protected class Node {
        ...
        public RSTile toRSTile(RSTile baseTile) {
            return new RSTile(ctx, x + baseTile.getWorldLocation().getX(), y + baseTile.getWorldLocation().getY(), baseTile.getWorldLocation().getPlane());
        }
    }
    ...
    private RSTile[] findPath(RSTile start, RSTile end, boolean remote) {
        ...
        if (ctx.client.getCollisionMaps() != null) {
            flags = ctx.client.getCollisionMaps()[plane].getFlags();
        }
        // loaded region only
    ...
    private RSTile pull(RSTile tile) {
        ...
        return new RSTile(ctx, x, y, p.getWorldLocation().getPlane());
    }
